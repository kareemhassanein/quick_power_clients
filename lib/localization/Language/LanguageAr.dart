
import 'Languages.dart';

class LanguageAr extends Languages {
  @override
  // TODO: implement appName
  String get appName => 'وقودي';

  @override
  // TODO: implement langName
  String get langName => 'العربية';


  @override
  // TODO: implement addNewStation
  String get addNewStation => 'إضافة محطة جديدة';

  @override
  // TODO: implement address
  String get address => 'العنوان';

  @override
  // TODO: implement alreadyHaveAccount
  String get alreadyHaveAccount => 'هل لديك حساب؟';

  @override
  // TODO: implement areYouSureToCancelOrder
  String get areYouSureToCancelOrder => 'هل أنت متأكد من إلغاء الطلب';

  @override
  // TODO: implement areYouSureToLogOut
  String get areYouSureToLogOut => 'هل أنت متأكد من تسجيل الخروج؟';

  @override
  // TODO: implement cancel
  String get cancel => 'إلغاء';

  @override
  // TODO: implement cancelOrder
  String get cancelOrder => 'إلغاء الطلب';

  @override
  // TODO: implement changePassword
  String get changePassword => 'تغيير كلمة المرور';

  @override
  // TODO: implement confirmNewPassword
  String get confirmNewPassword => 'تأكيد كلمة المرور الجديدة';

  @override
  // TODO: implement confirmPassword
  String get confirmPassword => 'تأكيد كلمة المرور';

  @override
  // TODO: implement date
  String get date => 'التاريخ';

  @override
  // TODO: implement done
  String get done => 'منتهي';

  @override
  // TODO: implement dontHaveAnAccount
  String get dontHaveAnAccount => 'ليس لديك حساب؟';

  @override
  // TODO: implement dueDate
  String get dueDate => 'تاريخ الاستحقاق';

  @override
  // TODO: implement enterPassword
  String get enterPassword => 'أدخل كلمة المرور';

  @override
  // TODO: implement forgerPassword
  String get forgerPassword =>  'نسيت كلمة المرور';

  @override
  // TODO: implement hello
  String get hello => 'هلا';

  @override
  // TODO: implement inProgress
  String get inProgress => 'فى تقدم';

  @override
  // TODO: implement invoiceDetails
  String get invoiceDetails => 'تفاصيل الفاتورة';

  @override
  // TODO: implement l
  String get l => 'لتر';

  @override
  // TODO: implement location
  String get location => 'الموقع';

  @override
  // TODO: implement logOut
  String get logOut => 'تسجيل خروج';

  @override
  // TODO: implement myProfile
  String get myProfile => 'الحساب الشخصي';

  @override
  // TODO: implement name
  String get name => 'الاسم';

  @override
  // TODO: implement newPassword
  String get newPassword => 'كلمة المرور الجديدة';

  @override
  // TODO: implement no
  String get no => 'لا';

  @override
  // TODO: implement noOrders
  String get noOrders => 'لا توجد أوامر';

  @override
  // TODO: implement orderDetails
  String get orderDetails => 'تفاصيل الطلب';

  @override
  // TODO: implement orderStatus
  String get orderStatus => 'حالة الطلب';

  @override
  // TODO: implement pending
  String get pending => 'قيد الانتظار';

  @override
  // TODO: implement phone
  String get phone => 'الجوال';

  @override
  // TODO: implement plsSignIn
  String get plsSignIn => 'الرجاء تسجيل الدخول إلى حسابك للمتابعة \n مع التطبيق.';

  @override
  // TODO: implement plsSignUp
  String get plsSignUp => 'يرجى التسجيل في حسابك للمتابعة \n مع التطبيق.';

  @override
  // TODO: implement productDetails
  String get productDetails => 'تفاصيل المنتج';

  @override
  // TODO: implement profile
  String get profile => 'حسابي';

  @override
  // TODO: implement quantity
  String get quantity => 'الكمية';

  @override
  // TODO: implement required
  String get required => 'مطلوب';

  @override
  // TODO: implement sar
  String get sar => 'ر.س';

  @override
  // TODO: implement save
  String get save => 'حفظ';

  @override
  // TODO: implement signIn
  String get signIn => 'تسجيل دخول';

  @override
  // TODO: implement signUp
  String get signUp => 'تسجيل جديد';

  @override
  // TODO: implement stations
  String get stations => 'المحطات';

  @override
  // TODO: implement subTotal
  String get subTotal => 'المجموع الفرعي';

  @override
  // TODO: implement submit
  String get submit => 'تأكيد';

  @override
  // TODO: implement total
  String get total => 'الإجمالي';

  @override
  // TODO: implement vat
  String get vat => 'ضريبة';

  @override
  // TODO: implement yesCancel
  String get yesCancel => 'نعم، إلغاء';

  @override
  // TODO: implement createNewOrder
  String get createNewOrder => 'إنشاء أمر جديد';

  @override
  // TODO: implement tryAgain
  String get tryAgain => 'حاول مرة اخرى';

  @override
  // TODO: implement productType
  String get productType => 'نوع المنتج';

  @override
  // TODO: implement changeLanguage
  String get changeLanguage => 'تغيير اللغة';

  @override
  // TODO: implement station
  String get station => 'محطة التفريغ';

  @override
  // TODO: implement noStationsAdded
  String get noStationsAdded => 'لا توجد محطات مضافة';

  @override
  // TODO: implement edit
  String get edit => 'التعديل';

  @override
  // TODO: implement stationLocation
  String get stationLocation => 'موقع المحطة';

  @override
  // TODO: implement email
  String get email => 'الإيميل';

  @override
  String get deliveryAddress => 'عنوان التسليم';

  @override
  String get deliveryDate => 'تاريخ الاستلام';

  @override
  String get exclusiveVAT => 'غير شامل ضريبة القيمة المضافة';

  @override
  String get inclusiveVAT => 'شامل ضريبة القيمة المضافة';

  @override
  String get requiredQuantity => 'الكمية المطلوبة باللتر';

  @override
  String get requiredProduct => 'المنتج المطلوب';

  @override
  String get confirmOrder => 'تأكيد الطلب';
  @override
  String get paymentMethod => 'وسيلة الدفع';

  @override
  // TODO: implement userId
  String get userId => 'رقم الهوية';

  @override
  // TODO: implement forgetPasswordHint
  String get forgetPasswordHint => 'لإعادة تعيين كلمة المرور الخاصة بك، يرجى إدخال رقم هاتفك المسجل لتلقى الرمز في رسالة نصية';

  @override
  // TODO: implement otpVerification
  String get otpVerification => 'التحقق من الرمز';

  @override
  // TODO: implement otpVerificationHint
  String get otpVerificationHint => 'أدخل رمز التحقق المكون من 4 أرقام المرسل إلى هاتفك';

  @override
  // TODO: implement resendAfter
  String get resendAfter => 'إعادة ارسال بعد';

  @override
  // TODO: implement seconds
  String get seconds => 'ث';

  @override
  // TODO: implement send
  String get send => 'ارسال';

  @override
  // TODO: implement recivedDate
  String get recivedDate => 'تاريخ التسليم';

  @override
  // TODO: implement driverInfo
  String get driverInfo => 'بيانات السائق';

  @override
  // TODO: implement expectedData
  String get expectedData => 'تاريخ الوصول المتوقع';

  @override
  // TODO: implement truckInfo
  String get truckInfo => 'بيانات الشاحنة';

  @override
  // TODO: implement invoiceNo
  String get invoiceNo => 'رقم الفاتورة';

  @override
  // TODO: implement reciverName
  String get reciverName => 'اسم المستلم';

  @override
  // TODO: implement vatNo
  String get vatNo => 'الرقم الضريبي';

  @override
  // TODO: implement notePriceAfter
  String get notePriceBefore => '*ملاحظة: الأسعار غير شاملة أجره النقل';

  @override
  // TODO: implement notePriceBefore
  String get  notePriceAfter=> '*ملاحظة: الأسعار شاملة أجره النقل';
  @override
  String get trackShipment => 'تتبع الشحن';
  @override
  String get trackingShipment => 'تتبع شحن';
  @override
  String get driverLocation => 'موقع السائق';

  @override
  // TODO: implement addNewAddress
  String get addNewAddress => 'اضافه عنوان';

  @override
  // TODO: implement delete
  String get delete => 'حذف';

  @override
  // TODO: implement deleteMyAccount
  String get deleteMyAccount => 'حذف الحساب';


  @override
  // TODO: implement deleteMyAccountHint
  String get deleteMyAccountHint => 'هل انت متأكد من حذف الحساب نهائياً';


  @override
  // TODO: implement faq
  String get faq => 'الأسئلة الشائعة';

  @override
  // TODO: implement firstName
  String get firstName => 'الاسم الأول';

  @override
  // TODO: implement lastName
  String get lastName => 'الاسم الأخير';

  @override
  // TODO: implement shareApp
  String get shareApp => 'مشاركة التطبيق';

  @override
  // TODO: implement termsAndConditions
  String get termsAndConditions => 'الشروط والأحكام';

  @override
  String get theme => "المظهر";
  @override
  String get systemMode => "الوضع النظامي";
  @override
  String get lightMode => "الوضع الفاتح";
  @override
  String get darkMode => "الوضع الداكن";
  @override
  String get chooseTheme => "اختر المظهر";

  @override
  String get loginRequerdMessage => 'تسجيل الدخول مطلوب لاستخدام هذه الخدمة';

  @override
  String get skip => 'تخطي';

  @override
  // TODO: implement aboutApp
  String get aboutApp => 'عن التطبيق';

  @override
  // TODO: implement aboutAppDesc
  String get aboutAppDesc => '''
كويك باور هو أداة تنظيمية شخصية تساعد المستخدمين على تسجيل وتتبع احتياجات الوقود لمواقعهم الخاصة بسهولة.

من خلال التطبيق، يمكن للمستخدم:
- إنشاء طلبات وقود لمواقعه المختلفة.
- متابعة حالة الطلب من لحظة الإنشاء وحتى الإتمام.
- إضافة وتنظيم المواقع الخاصة به.
- عرض سجل الطلبات السابقة وتفاصيل كل طلب.
- استقبال إشعارات عند تحديث حالة الطلب.
- تخصيص أسماء المواقع لتسهيل المتابعة والإدارة.

يتطلب التطبيق تسجيل الدخول عند محاولة المستخدم إنشاء طلب جديد أو إدارة مواقعه الخاصة، وذلك لأن هذه العمليات تعتمد على بيانات مرتبطة بحساب المستخدم لضمان تتبع دقيق لكل طلب.

''';
}
